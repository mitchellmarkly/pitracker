...
export const normalizeHeatmapPayload = (payload: unknown) => {
  ...
  if (!parsed.success) {
    return { ok: false as const, errors: parsed.error.issues.map((i: { path: (string|number)[]; message: string }) => `${i.path.join('.')}: ${i.message}`) };
  }
  return { ok: true as const, data: parsed.data };
};

export const getNextOpenSlot = (slotsTotal: number, assignments: Pick<Assignment, 'slot' | 'active'>[]) => {
...
export const hasDuplicatePlanetForCharacter = (
  assignments: Pick<Assignment, 'characterId' | 'system' | 'planet' | 'active'>[],
  characterId: string,
  system: string,
  planet: string,
) => assignments.some((a) => a.active && a.characterId === characterId && a.system === system && a.planet === planet);

const ymdToday = () => new Date().toISOString().slice(0, 10);

export const parseYieldPaste = (input: string) => {
  const lines = input.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  const parsed: { date: string; product: string; amount: number }[] = [];
  const errors: string[] = [];
